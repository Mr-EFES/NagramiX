...
Android-приложение строится отдельно на базе текущего проверенного официального `DrKLO/Telegram` 12.10.1. CI получает официальный Android upstream, применяет `android/apply_overlay.py` и собирает APK; Android-код не связывается с приложением для iOS. NagramX 1258 служит только каталогом продуктовых идей.
...
4. На чистое дерево исходников накладывается изолированный слой из каталога `ios/`.
