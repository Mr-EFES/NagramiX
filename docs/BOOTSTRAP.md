...
Android-приложение строится отдельно на базе закреплённого официального `DrKLO/Telegram` 12.9.2. CI получает официальный Android upstream, применяет `android/apply_overlay.py` и собирает APK; Android-код не связывается с приложением для iOS. NagramX 1258 служит только каталогом продуктовых идей.
