...
## Актуальная версия: 0.3.8

**0.3.8 — кандидат в стабильную базовую версию проекта для проверки на
физическом iPhone.** Устаревшие release-файлы в рабочем дереве не хранятся.

### Реализовано в iOS

- категории настроек «Интерфейс», «Функции» и «Прочее» с глобальным поиском;
...
- русский экран первого запуска с продолжением на языке iPhone и тёмная тема по умолчанию;
- максимально широкие посты во всех обычных лентах каналов при включённой настройке с сохранением штатных реакций;
- защита offline/proxy запуска.

Версия 0.3.8 закреплена как единственная актуальная линия NagramiX. Для неё
подготовлены авторитетная ARM64-сборка и упаковка unsigned IPA в GitHub Actions.
Статус stable присваивается только после успешной сборки и полного
runtime-регресса на физическом iPhone.
