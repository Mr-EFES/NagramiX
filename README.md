...
## Текущая версия: 0.2.5 pre-release
...
Нативная iOS-компиляция 0.2.5 пройдена. Полная физическая проверка на iPhone остаётся обязательной для runtime-утверждений.

### Статус Android

Android строится на официальной базе Telegram. Исходный паритет 0.2.5 подготовлен; ARM64 APK workflow доступен только для ручного запуска. Сборка и физическая Android-проверка ещё не выполнены.
